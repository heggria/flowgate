...
var require_make_client = __commonJS({
  "../../node_modules/@grpc/grpc-js/build/src/make-client.js"(exports2) {
    ...
    exports2.loadPackageDefinition = loadPackageDefinition3;
    ...
    function loadPackageDefinition3(packageDef) {
      const result = {};
...
var require_converter = __commonJS({
  "../../node_modules/protobufjs/src/converter.js"(exports2) {
    ...
    converter.fromObject = function fromObject(mtype) {
      ...
      if (!fields2.length) return gen("return new this.ctor");
      ...
      for (var i = 0; i < fields2.length; ++i) {
        var field = fields2[i].resolve(), prop = util.safeProp(field.name);
        if (field.map) {
...
var require_root = __commonJS({
  "../../node_modules/protobufjs/src/root.js"(exports2, module2) {
    ...
    Root.prototype.loadSync = function loadSync3(filename, options) {
      if (!util.isNode)
...
var require_index_light = __commonJS({
  "../../node_modules/protobufjs/src/index-light.js"(exports2, module2) {
    ...
    protobuf.load = load;
    function loadSync3(filename, root) {
      if (!root)
        root = new protobuf.Root();
      return root.loadSync(filename);
    }
    protobuf.loadSync = loadSync3;
...
var require_src2 = __commonJS({
  "../../node_modules/@grpc/proto-loader/build/src/index.js"(exports2) {
    ...
    exports2.load = load;
    function loadSync3(filename, options) {
      const loadedRoot = (0, util_1.loadProtosWithOptionsSync)(filename, options);
      return createPackageDefinition(loadedRoot, options);
    }
    exports2.loadSync = loadSync3;
...
var require_transport = __commonJS({
  "../../node_modules/@grpc/grpc-js/build/src/transport.js"(exports2) {
    ...
    var Http2Transport = class {
      constructor(session2, subchannelAddress, options, remoteName) {
        ...
        session2.once("error", (error) => {
          this.trace("connection closed with error " + error.message);
          this.handleDisconnect();
        });
        session2.socket.once("close", (hadError) => {
          this.trace("connection closed. hadError=" + hadError);
          this.handleDisconnect();
        });
        if (logging.isTracerEnabled(TRACER_NAME)) {
          session2.on("remoteSettings", (settings) => {
            this.trace("new settings received" + (this.session !== session2 ? " on the old connection" : "") + ": " + JSON.stringify(settings));
          });
          session2.on("localSettings", (settings) => {
            this.trace("local settings acknowledged by remote" + (this.session !== session2 ? " on the old connection" : "") + ": " + JSON.stringify(settings));
          });
        }
        if (this.keepaliveWithoutCalls) {
          this.maybeStartKeepalivePingTimer();
        }
        if (session2.socket instanceof tls_1.TLSSocket) {
          this.authContext = {
            transportSecurityType: "ssl",
            sslPeerCertificate: session2.socket.getPeerCertificate()
          };
        } else {
          this.authContext = {};
        }
    ...
    var Http2SubchannelConnector = class {
      ...
      createSession(secureConnectResult, address, options) {
        ...
        return new Promise((resolve, reject) => {
          ...
          const session2 = http2.connect(`${scheme}://${targetPath}`, sessionOptions);
          ...
          session2.unref();
          session2.once("remoteSettings", () => {
            var _a2;
            if (connWin && connWin > defaultWin) {
              try {
                session2.setLocalWindowSize(connWin);
              } catch (_b2) {
                const delta = connWin - ((_a2 = session2.state.localWindowSize) !== null && _a2 !== void 0 ? _a2 : defaultWin);
                if (delta > 0)
                  session2.incrementWindowSize(delta);
              }
            }
            session2.removeAllListeners();
            secureConnectResult.socket.removeListener("close", closeHandler);
            secureConnectResult.socket.removeListener("error", errorHandler);
            resolve(new Http2Transport(session2, address, options, remoteName));
            this.session = null;
          });
          session2.once("close", closeHandler);
          session2.once("error", errorHandler);
  ...
  "../../node_modules/@grpc/grpc-js/build/src/server.js"(exports2) {
    ...
    var Server = (() => {
      ...
      return _a = class Server {
        ...
        getChannelzSessionInfo(session2) {
          var _b, _c, _d;
          const sessionInfo = this.sessions.get(session2);
          const sessionSocket = session2.socket;
          const remoteAddress = sessionSocket.remoteAddress ? (0, subchannel_address_1.stringToSubchannelAddress)(sessionSocket.remoteAddress, sessionSocket.remotePort) : null;
          const localAddress = sessionSocket.localAddress ? (0, subchannel_address_1.stringToSubchannelAddress)(sessionSocket.localAddress, sessionSocket.localPort) : null;
          let tlsInfo;
          if (session2.encrypted) {
            const tlsSocket = sessionSocket;
            const cipherInfo = tlsSocket.getCipher();
            const certificate = tlsSocket.getCertificate();
            const peerCertificate = tlsSocket.getPeerCertificate();
          ...
          const socketInfo = {
            ...
            localFlowControlWindow: (_c = session2.state.localWindowSize) !== null && _c !== void 0 ? _c : null,
            remoteFlowControlWindow: (_d = session2.state.remoteWindowSize) !== null && _d !== void 0 ? _d : null
          };
          return socketInfo;
        }
        trace(text) {
        ...
        resolvePort(port2) {
          return new Promise((resolve, reject) => {
            ...
            const resolverListener = (endpointList, attributes, serviceConfig, resolutionNote) => {
              ...
              if (addressList.length === 0) {
                reject(new Error(`No addresses resolved for port ${port2}`));
                return true;
              }
              resolve(addressList);
              return true;
            };
            const resolver = (0, resolver_1.createResolver)(port2, resolverListener, this.options);
            resolver.updateResolution();
          });
        }
        async bindPort(port2, boundPortObject) {
          const addressList = await this.resolvePort(port2);
        ...
        normalizePort(port2) {
          const initialPortUri = (0, uri_parser_1.parseUri)(port2);
        ...
        bindAsync(port2, creds, callback) {
          ...
          this.trace("bindAsync port=" + port2);
          const portUri = this.normalizePort(port2);
          const deferredCallback = (error, port3) => {
            process.nextTick(() => callback(error, port3));
          };
          let boundPortObject = this.boundPorts.get((0, uri_parser_1.uriToString)(portUri));
          if (boundPortObject) {
            if (!creds._equals(boundPortObject.credentials)) {
              deferredCallback(new Error(`${port2} already bound with incompatible credentials`), 0);
              return;
            }
        ...
        experimentalCreateConnectionInjectorWithChannelzRef(credentials3, channelzRef, ownsChannelzRef = false) {
          ...
          return {
            ...
            drain: (graceTimeMs) => {
              var _b, _c;
              for (const session2 of sessionsSet) {
        ...
        closeSession(session2, callback) {
          var _b;
          this.trace("Closing session initiated by " + ((_b = session2.socket) === null || _b === void 0 ? void 0 : _b.remoteAddress));
          const sessionInfo = this.sessions.get(session2);
          const closeCallback = () => {
            if (sessionInfo) {
        ...
        completeUnbind(boundPortObject) {
          for (const server of boundPortObject.listeningServers) {
            ...
            if (serverInfo) {
              for (const session2 of serverInfo.sessions) {
                this.closeSession(session2);
              }
            }
          }
         ...
         * @returns
         */
        drain(port2, graceTimeMs) {
          var _b, _c;
          this.trace("drain port=" + port2 + " graceTimeMs=" + graceTimeMs);
          const portUri = this.normalizePort(port2);
          const splitPort = (0, uri_parser_1.splitHostPort)(portUri.path);
          if ((splitPort === null || splitPort === void 0 ? void 0 : splitPort.port) === 0) {
          ...
          for (const http2Server of boundPortObject.listeningServers) {
            ...
            if (serverEntry) {
              for (const session2 of serverEntry.sessions) {
                allSessions.add(session2);
                this.closeSession(session2, () => {
                  allSessions.delete(session2);
                });
              }
            }
          }
          (_c = (_b = setTimeout(() => {
            for (const session2 of allSessions) {
              session2.destroy(http2.constants.NGHTTP2_CANCEL);
            }
          }, graceTimeMs)).unref) === null || _c === void 0 ? void 0 : _c.call(_b);
        }
        forceShutdown() {
          ...
          this.sessions.forEach((channelzInfo, session2) => {
            this.closeSession(session2);
            session2.destroy(http2.constants.NGHTTP2_CANCEL);
          });
          this.sessions.clear();
        ...
        tryShutdown(callback) {
          ...
          for (const [serverKey, server] of this.http2Servers.entries()) {
            ...
            for (const session2 of server.sessions.keys()) {
              pendingChecks++;
              const sessionString = (_b = session2.socket) === null || _b === void 0 ? void 0 : _b.remoteAddress;
        ...
        _sessionHandler(http2Server) {
          return (session2) => {
            var _b, _c;
            (_b = this.http2Servers.get(http2Server)) === null || _b === void 0 ? void 0 : _b.sessions.add(session2);
            let connectionAgeTimer = null;
            let connectionAgeGraceTimer = null;
            let keepaliveTimer = null;
            let sessionClosedByServer = false;
            const idleTimeoutObj = this.enableIdleTimeout(session2);
            if (this.maxConnectionAgeMs !== UNLIMITED_CONNECTION_AGE_MS) {
              ...
              connectionAgeTimer = setTimeout(() => {
                ...
                this.trace("Connection dropped by max connection age: " + ((_b2 = session2.socket) === null || _b2 === void 0 ? void 0 : _b2.remoteAddress));
                try {
                  session2.goaway(http2.constants.NGHTTP2_NO_ERROR, ~(1 << 31), kMaxAge);
                } catch (e) {
                  session2.destroy();
                  return;
                }
            ...
            const canSendPing = () => {
              return !session2.destroyed && this.keepaliveTimeMs < KEEPALIVE_MAX_TIME_MS && this.keepaliveTimeMs > 0;
            };
            let sendPing;
            ...
            sendPing = () => {
              ...
              let pingSendError = "";
              try {
                const pingSentSuccessfully = session2.ping((err, duration, payload) => {
                  clearKeepaliveTimeout();
                  if (err) {
                    this.keepaliveTrace("Ping failed with error: " + err.message);
                    sessionClosedByServer = true;
            ...
            session2.on("close", () => {
              ...
              if (!sessionClosedByServer) {
                this.trace(`Connection dropped by client ${(_b2 = session2.socket) === null || _b2 === void 0 ? void 0 : _b2.remoteAddress}`);
              }
              ...
              if (idleTimeoutObj !== null) {
                ...
                this.sessionIdleTimeouts.delete(session2);
              }
              (_c2 = this.http2Servers.get(http2Server)) === null || _c2 === void 0 ? void 0 : _c2.sessions.delete(session2);
            });
          };
        }
        _channelzSessionHandler(http2Server) {
          return (session2) => {
            var _b, _c, _d, _e;
            const channelzRef = (0, channelz_1.registerChannelzSocket)((_c = (_b = session2.socket) === null || _b === void 0 ? void 0 : _b.remoteAddress) !== null && _c !== void 0 ? _c : "unknown", this.getChannelzSessionInfo.bind(this, session2), this.channelzEnabled);
            ...
            (_d = this.http2Servers.get(http2Server)) === null || _d === void 0 ? void 0 : _d.sessions.add(session2);
            this.sessions.set(session2, channelzSessionInfo);
            const clientAddress = `${session2.socket.remoteAddress}:${session2.socket.remotePort}`;
            ...
            session2.on("close", () => {
              ...
              (_b2 = this.http2Servers.get(http2Server)) === null || _b2 === void 0 ? void 0 : _b2.sessions.delete(session2);
              this.sessions.delete(session2);
            });
          };
        }
        enableIdleTimeout(session2) {
          var _b, _c;
          ...
          const idleTimeoutObj = {
            activeStreams: 0,
            lastIdle: Date.now(),
            onClose: this.onStreamClose.bind(this, session2),
            timeout: setTimeout(this.onIdleTimeout, this.sessionIdleTimeout, this, session2)
          };
          (_c = (_b = idleTimeoutObj.timeout).unref) === null || _c === void 0 ? void 0 : _c.call(_b);
          this.sessionIdleTimeouts.set(session2, idleTimeoutObj);
          const { socket } = session2;
          this.trace("Enable idle timeout for " + socket.remoteAddress + ":" + socket.remotePort);
          return idleTimeoutObj;
        }
        onIdleTimeout(ctx, session2) {
          const { socket } = session2;
          const sessionInfo = ctx.sessionIdleTimeouts.get(session2);
        ...
        onStreamOpened(stream) {
          ...
          const idleTimeoutObj = this.sessionIdleTimeouts.get(session2);
          if (idleTimeoutObj) {
            idleTimeoutObj.activeStreams += 1;
            stream.once("close", idleTimeoutObj.onClose);
          }
        }
        onStreamClose(session2) {
          var _b, _c;
          const idleTimeoutObj = this.sessionIdleTimeouts.get(session2);
          if (idleTimeoutObj) {
            idleTimeoutObj.activeStreams -= 1;
            if (idleTimeoutObj.activeStreams === 0) {
              ...
              this.trace("Session onStreamClose" + ((_b = session2.socket) === null || _b === void 0 ? void 0 : _b.remoteAddress) + ":" + ((_c = session2.socket) === null || _c === void 0 ? void 0 : _c.remotePort) + " at " + idleTimeoutObj.lastIdle);
            }
          }
        }
      }, (() => {
...
// packages/runtime/src/trace-context.ts
var import_node_async_hooks = require("node:async_hooks");
var requestTrace = new import_node_async_hooks.AsyncLocalStorage();

// packages/runtime/src/request-scope.ts
...
// packages/contracts/src/builtin-catalog.json
var builtin_catalog_default = [
  {
    id: "builtin.subscription",
    name: "\u8BA2\u9605\u4E0E\u8282\u70B9\u89E3\u6790",
    description: "\u5BFC\u5165\u548C\u5237\u65B0 HTTPS \u8BA2\u9605\uFF0C\u5C06\u53D7\u652F\u6301\u7684\u914D\u7F6E\u8F6C\u6362\u4E3A\u8282\u70B9\u3002",
    version: "1.0.0",
    required: true,
    defaultEnabled: true,
    dependencies: [],
    capabilities: [],
    permissions: ["subscription.https.read"],
    contributions: ["command"],
    methods: ["subscription.parse", "subscription.fetch"]
  },
  {
    id: "builtin.rules",
    name: "\u89C4\u5219\u96C6\u89E3\u6790",
    description: "\u8BFB\u53D6\u57DF\u540D\u5217\u8868\u548C\u53D7\u652F\u6301\u7684 sing-box \u89C4\u5219\u96C6\uFF0C\u6821\u9A8C\u540E\u66F4\u65B0\u5206\u6D41\u89C4\u5219\u3002",
    version: "1.0.0",
    required: true,
    defaultEnabled: true,
    dependencies: [],
    ...
    permissions: [],
    contributions: ["command"],
    methods: ["ruleset.parse"]
  },
  {
    id: "builtin.network",
    name: "\u7CFB\u7EDF\u7F51\u7EDC\u53D1\u73B0",
    description: "\u89C2\u5BDF\u672C\u673A\u63A5\u53E3\u3001\u8DEF\u7531\u3001DNS \u4E0E\u7CFB\u7EDF\u4EE3\u7406\uFF0C\u4E3A\u7F51\u7EDC\u72B6\u6001\u548C\u51B2\u7A81\u63D0\u793A\u63D0\u4F9B\u4F9D\u636E\u3002",
    version: "1.0.0",
    required: true,
    defaultEnabled: true,
    dependencies: [],
    capabilities: [],
    permissions: ["system.network.read"],
    contributions: ["command"],
    methods: ["network.inspect"]
  },
  {
    id: "builtin.tailscale",
    name: "Tailscale \u72B6\u6001\u8BCA\u65AD",
    description: "\u901A\u8FC7\u672C\u673A\u5E94\u7528\u548C CLI \u8BFB\u53D6 Tailscale \u8FD0\u884C\u72B6\u6001\uFF1B\u4E0D\u8BFB\u53D6 Peer \u6E05\u5355\u6216\u4FEE\u6539 VPN \u8BBE\u7F6E\u3002",
    version: "1.0.0",
    required: false,
    defaultEnabled: true,
    dependencies: ["builtin.network"],
    capabilities: [],
    permissions: ["application.status.read"],
    contributions: ["command"],
    methods: ["diagnostic.tailscale"]
  },
  {
    id: "builtin.singbox",
    name: "GUI.for.SingBox \u72B6\u6001\u8BCA\u65AD",
    description: "\u68C0\u67E5\u6807\u51C6\u5B89\u88C5\u76EE\u5F55\u548C\u76F8\u5173\u8FDB\u7A0B\uFF0C\u8F85\u52A9\u5224\u65AD\u5916\u90E8\u4EE3\u7406\u662F\u5426\u8FD0\u884C\uFF1B\u4E0D\u8BFB\u53D6\u8BA2\u9605\u6216\u63A7\u5236\u4EE4\u724C\u3002",
    version: "1.0.0",
    required: false,
    defaultEnabled: true,
    dependencies: ["builtin.network"],
    ...
    permissions: ["application.status.read"],
    contributions: ["command"],
    methods: ["diagnostic.singbox"]
  }
];

// packages/contracts/src/extensions.ts
var builtinExtensions = builtin_catalog_default;
function extensionPreferences(value) {
...
function validateConfiguration(c) {
  if (c.schema !== 1 || !Number.isSafeInteger(c.revision) || !Array.isArray(c.nodes) || c.nodes.length > 5e3 || !Array.isArray(c.rules) || c.rules.length > 1e4 || !Array.isArray(c.subscriptions))
    throw new Error("\u914D\u7F6E\u7ED3\u6784\u65E0\u6548");
  if (!c.settings || typeof c.settings.autoConnect !== "boolean" || !["manual", "system", "tun"].includes(c.settings.mode) || !Number.isInteger(c.settings.listenPort) || c.settings.listenPort < 1024 || c.settings.listenPort > 65535)
    throw new Error("\u76D1\u542C\u7AEF\u53E3\u5FC5\u987B\u5728 1024\u201365535 \u4E4B\u95F4");
  const ids = /* @__PURE__ */ new Set(["direct", "block", "select"]);
  for (const n of c.nodes) {
    validateNode(n);
    if (["localhost", "127.0.0.1", "::1", "[::1]"].includes(n.server) && n.port === c.settings.listenPort)
      throw new Error("\u8282\u70B9\u6307\u5411\u672C\u5E94\u7528\u76D1\u542C\u7AEF\u53E3\uFF0C\u4F1A\u5F62\u6210\u4EE3\u7406\u5FAA\u73AF");
    if (ids.has(n.id)) throw new Error("\u8282\u70B9\u6807\u8BC6\u91CD\u590D");
    ids.add(n.id);
  }
  for (const network of c.externalNetworks ?? []) {
    if (!/^[\w.-]{1,100}$/.test(network.id) || ids.has(network.id) || !network.name || !/^[a-zA-Z][a-zA-Z0-9]{0,20}$/.test(network.interface))
      throw new Error("\u5916\u90E8\u7F51\u7EDC\u914D\u7F6E\u65E0\u6548");
    const resolver = new URL(network.dnsServer);
    if (!["udp:", "tls:", "https:"].includes(resolver.protocol) || resolver.username || resolver.password)
      throw new Error("\u5916\u90E8\u7F51\u7EDC DNS \u65E0\u6548");
    ids.add(network.id);
  }
  if (!ids.has(c.settings.selectedNode) || c.settings.selectedNode === "select" || c.settings.selectedNode === "block")
    throw new Error("\u8BF7\u9009\u62E9\u6709\u6548\u8282\u70B9");
  if (!ids.has(c.settings.finalOutbound)) throw new Error("\u9ED8\u8BA4\u51FA\u53E3\u4E0D\u5B58\u5728");
  const dns = new URL(c.settings.dnsServer);
  if (!["https:", "tls:", "udp:"].includes(dns.protocol) || dns.username || dns.password)
    throw new Error("DNS \u5730\u5740\u987B\u4E3A HTTPS\u3001TLS \u6216 UDP");
  const sourceIds = /* @__PURE__ */ new Set();
  if (c.ruleSources && (!Array.isArray(c.ruleSources) || c.ruleSources.length > 100))
    throw new Error("\u89C4\u5219\u96C6\u6765\u6E90\u6570\u91CF\u65E0\u6548");
  for (const source of c.ruleSources ?? []) {
    if (!source || !/^[\w.-]{1,100}$/.test(source.id) || sourceIds.has(source.id) || typeof source.name !== "string" || !source.name.trim() || source.name.length > 200 || !["sing-box-json", "domain-list"].includes(source.format) || !ids.has(source.outbound))
  ...
  for (const r of c.rules) {
    if (!["domain_suffix", "domain", "ip_cidr", "process_name"].includes(
      r.kind
    ) || typeof r.value !== "string" || !r.value.trim() || r.value.length > 500 || !ids.has(r.outbound) || r.sourceId !== void 0 && !sourceIds.has(r.sourceId))
      throw new Error("\u5206\u6D41\u89C4\u5219\u65E0\u6548");
  }
}
function validateNode(n) {
  if (!n || !/^[\w.-]{1,100}$/.test(n.id) || typeof n.name !== "string" || !n.name.trim() || n.name.length > 200 || ![
    "http",
    "socks",
    "shadowsocks",
    "trojan",
    ...
    "hysteria2",
    "tuic"
  ].includes(n.type) || typeof n.server !== "string" || !n.server || /[\s/]/.test(n.server) || !Number.isInteger(n.port) || n.port < 1 || n.port > 65535 || !n.options || typeof n.options !== "object")
...
function compileConfiguration(c) {
  validateConfiguration(c);
  const dns = new URL(c.settings.dnsServer);
  const server = {
    type: dns.protocol === "https:" ? "https" : dns.protocol === "tls:" ? "tls" : "udp",
    tag: "resolver",
    server: dns.hostname,
    ...dns.port ? { server_port: Number(dns.port) } : {},
    ...dns.protocol === "https:" ? { path: dns.pathname || "/dns-query" } : {},
    ...c.settings.selectedNode !== "direct" ? { detour: c.settings.selectedNode } : {}
  };
  const networks = c.externalNetworks ?? [];
  const networkDNS = networks.map((network) => {
    const u = new URL(network.dnsServer);
    return {
      type: u.protocol === "https:" ? "https" : u.protocol === "tls:" ? "tls" : "udp",
      tag: "dns-" + network.id,
      server: u.hostname,
  ...
  const dnsRules = c.rules.filter(
    (r) => ["domain", "domain_suffix"].includes(r.kind) && networks.some((n) => n.id === r.outbound)
  ).map((r) => ({
    [r.kind]: [r.value],
    action: "route",
  ...
  return {
    ...
    route: {
      ...
      rules: [
        ...
        ...c.rules.map((r) => ({
          ...{ [r.kind]: [r.value] },
          ...r.outbound === "block" ? { action: "reject" } : { action: "route", outbound: r.outbound }
        })),
        ...c.settings.finalOutbound === "block" ? [{ action: "reject" }] : []
      ],
      final: c.settings.finalOutbound === "block" ? "direct" : c.settings.finalOutbound
    }
  };
}
function explainRoute(c, target) {
  const hit = c.rules.find(
    (r) => r.kind === "domain" ? target === r.value : r.kind === "domain_suffix" ? target === r.value || target.endsWith("." + r.value.replace(/^\./, "")) : false
  );
  return {
    target,
    rule: hit ?? null,
...
async function atomicWrite(path, value) {
  const tmp = path + ".tmp";
  const f = await (0, import_promises.open)(tmp, "w", 384);
  try {
...
var StateStore = class {
  ...
  async start(readonly = false) {
    await (0, import_promises.mkdir)(this.directory, { recursive: true, mode: 448 });
    if (!readonly) {
      this.lock = await (0, import_promises.open)((0, import_node_path.join)(this.directory, "writer.lock"), "wx", 384);
    ...
    try {
      const data = JSON.parse(
        await (0, import_promises.readFile)((0, import_node_path.join)(this.directory, "state.json"), "utf8")
      );
      ...
      if (!Number.isSafeInteger(this.extensionRevision) || this.extensionRevision < 0)
    ...
    this.operations = this.operations.map(
      (o) => o.state === "pending" ? { ...o, state: "unknown" } : o
    );
  }
  async persist() {
    if (!this.lock) throw new Error("No writer lease");
    await atomicWrite((0, import_node_path.join)(this.directory, "state.json"), {
  ...
  async close() {
    ...
    if (this.lock) {
      ...
      this.lock = void 0;
      await (0, import_promises.unlink)((0, import_node_path.join)(this.directory, "writer.lock"));
    }
  }
};

// packages/service/src/kernel/measurement.ts
var import_grpc_js = __toESM(require_src3(), 1);
var import_proto_loader = __toESM(require_src2(), 1);
var import_node_path2 = require("node:path");
async function measureOutbound(control, id, signal, schemaPath = (0, import_node_path2.join)(__dirname, "telemetry.proto")) {
  const definition = (0, import_grpc_js.loadPackageDefinition)(
    (0, import_proto_loader.loadSync)(schemaPath, { longs: String, defaults: true })
  );
  const Service = definition.daemon.StartedService;
  ...
  let started = Math.floor(Date.now() / 1e3);
  return new Promise((resolve, reject) => {
    const stream = client.subscribeOutbounds({}, metadata);
    let armed = false, dispatched = false, finished = false, previousTime = 0;
    let triggerTimer;
    ...
    stream.on("data", (data) => {
      const outbound = data.outbounds?.find((item) => item.tag === id);
      if (!outbound) {
      ...
      if (!armed) {
        ...
        const dispatch = () => {
          if (finished) return;
          started = Math.floor(Date.now() / 1e3);
          dispatched = true;
          client.urlTest(
            { outboundTag: id },
            metadata,
        ...
        if (previousTime >= started)
          triggerTimer = setTimeout(dispatch, 1010 - Date.now() % 1e3);
        else dispatch();
        return;
      }
      if (dispatched && Number(outbound.urlTestTime) >= started && Number(outbound.urlTestTime) > previousTime)
        complete(void 0, {
          id,
          state: "succeeded",
          delayMs: Number(outbound.urlTestDelay),
          measuredAt: new Date(
            Number(outbound.urlTestTime) * 1e3
          ).toISOString()
        });
    });
...
// packages/service/src/handoff.ts
var import_promises2 = require("node:fs/promises");
var import_node_path3 = require("node:path");
async function readHandoff(directory, revision) {
  try {
    const handoff = JSON.parse(
      await (0, import_promises2.readFile)((0, import_node_path3.join)(directory, "handoff.json"), "utf8")
    );
    if (handoff.version !== 1 || handoff.protocol !== 1 || handoff.schema !== 1 || !Number.isSafeInteger(handoff.revision) || handoff.revision > revision || !Array.isArray(handoff.pendingOperationIds))
...
async function writeHandoff(directory, value) {
  await atomicWrite((0, import_node_path3.join)(directory, "handoff.json"), value);
}
...
var package_default = {
  ...
  version: "0.2.0",
  private: true,
  description: "FlowGate modular proxy workspace",
  main: "dist/main.cjs",
  scripts: {
    ...
    "test:native": "npm run build && tsx tests/native.integration.ts",
    "test:gateway": "npm run build && tsx tests/gateway.integration.ts",
    "test:updates": "npm run build && node tests/update.integration.mjs",
    "package:mac": "node scripts/package-macos.mjs",
    verify: "npm run typecheck && npm run check:boundaries && npm test && npm run build && node tests/e2e.mjs && tsx tests/native.integration.ts && tsx tests/protocols.integration.ts && tsx tests/native-faults.integration.ts && node tests/ownership.integration.mjs && tsx tests/gateway.integration.ts && node tests/update.integration.mjs && node tests/credentials.integration.mjs && node tests/cancellation.integration.mjs && node tests/features.integration.mjs && node tests/drafts.integration.mjs && node tests/journeys.integration.mjs && node tests/ui-design.integration.mjs && node tests/extensions.integration.mjs && node tests/ui.integration.mjs",
    "test:ui": "npm run build && node tests/ui.integration.mjs",
    "setup:kernel": "node scripts/fetch-kernel.mjs",
    "test:journeys": "npm run build && node tests/journeys.integration.mjs",
    "test:extensions": "npm run build && node tests/extensions.integration.mjs",
    "test:ui-design": "npm run build && node tests/ui-design.integration.mjs",
    "test:infrastructure": "node --test tests/infrastructure.test.mjs",
    "audit:dependencies": "node scripts/audit-dependencies.mjs",
    "test:package": "node tests/package.integration.mjs"
  },
  devDependencies: {
...
var BUILD_VERSION = package_default.version;

// packages/domain/src/network-conflicts.ts
function networkConflicts(c, observed, kernel) {
  if (!observed) return [];
  const issues = [];
  ...
  if (c.settings.mode === "system" && observed.proxyEnabled && !kernel.systemControl)
    issues.push({
      id: "existing-proxy",
      severity: "blocked",
      message: "\u7CFB\u7EDF\u5DF2\u542F\u7528\u5176\u4ED6\u4EE3\u7406\u6216 PAC\u3002\u8BF7\u5148\u5728\u6240\u5C5E\u5E94\u7528\u4E2D\u5173\u95ED\u63A5\u7BA1\uFF0C\u6216\u4F7F\u7528\u624B\u52A8\u4EE3\u7406\u6A21\u5F0F\u3002"
    });
  if (c.settings.mode === "tun" && observed.defaultInterface?.startsWith("utun"))
    issues.push({
      id: "tunnel-default",
      severity: "warning",
      message: "\u9ED8\u8BA4\u51FA\u53E3\u4E3A\u96A7\u9053\u63A5\u53E3\u3002\u65E0\u6CD5\u4EC5\u51ED\u63A5\u53E3\u786E\u8BA4\u5F3A\u5236 VPN \u7B56\u7565\uFF1BTUN \u63A5\u7BA1\u524D\u9700\u8981\u9A8C\u8BC1\u8DEF\u7531\u548C DNS \u5171\u5B58\u3002"
    });
  return issues;
}

// packages/service/src/core.ts
var import_node_crypto = require("node:crypto");

// packages/service/src/network-observer.ts
var NetworkObserver = class {
  constructor(inspect, publish, failed) {
  ...
  refresh() {
    ...
    this.pending = Promise.resolve().then(this.inspect).then(
      (state) => {
...
// packages/service/src/core.ts
var ServiceCore = class {
  constructor(store, native, extension, releaseSet, telemetry, version = BUILD_VERSION) {
    this.store = store;
    this.native = native;
    ...
    this.networkObserver = new NetworkObserver(
      async () => await this.extension("network.inspect"),
      (state) => {
        this.network = state;
      },
      () => {
  ...
  async readExtensions() {
    try {
      ...
      this.extensionStates = builtinExtensions.map((descriptor) => {
        ...
        if (!observed || !["starting", "ready", "draining", "stopped", "failed"].includes(
          observed.status
        ))
        ...
        return {
          ...
          desiredEnabled: this.store.extensionPreferences[descriptor.id] ?? descriptor.defaultEnabled,
  ...
  async start() {
    ...
    for (const o of this.store.operations) {
      if (o.state === "unknown" && o.kind === "proxy.connect" && kernel.appliedRevision === o.revision && kernel.operationId === o.id) {
        o.state = "succeeded";
        o.completedAt = (/* @__PURE__ */ new Date()).toISOString();
      }
    }
    ...
    try {
      const health = await this.extension("health");
      if (Array.isArray(health?.modules))
        this.moduleInfo = health.modules.map((module2) => ({
          ...module2,
          name: module2.id === "builtin.network" ? "\u7F51\u7EDC\u53D1\u73B0" : module2.id === "builtin.subscription" ? "\u8BA2\u9605\u89E3\u6790" : module2.id
        }));
    } catch {
    }
    this.lifecycle = "ready";
    this.networkObserver.start();
    if (this.store.configuration.settings.autoConnect && kernel.status === "stopped") {
      await this.request(
        "proxy.connect",
        {},
  ...
  async request(method, payload, operationId, signal) {
    ...
    if (method === "node.measure") {
      ...
      try {
        const kernel = await this.native.status(), control = await this.native.control?.();
        if (kernel.status !== "running" || kernel.appliedRevision !== revision || !control)
    ...
    return this.store.transact(async () => {
      ...
      const operation = {
        id: operationId,
        traceId: requestTrace.getStore()?.traceId ?? (0, import_node_crypto.randomUUID)().replaceAll("-", ""),
        kind: method,
        state: "pending",
      ...
      try {
        if (method === "extensions.setEnabled" || method === "extensions.restart") {
          ...
          await this.extension(
            method === "extensions.restart" ? "extensions.restart" : "extensions.configure",
            { preferences: next }
          );
          ...
          if (this.store.configuration.settings.mode !== "manual" && !(await this.native.status()).systemControl)
          ...
          if (method === "configuration.save") {
            ...
            if (payload.externalNetworks !== void 0)
          ...
          if (method === "subscription.remove") {
            ...
            const removed = new Set(
              next.nodes.filter((n) => n.sourceId === payload.id).map((n) => n.id)
            );
            next.subscriptions = next.subscriptions.filter(
              (s) => s.id !== payload.id
            );
            next.nodes = next.nodes.map(
              (n) => removed.has(n.id) ? { ...n, sourceId: void 0 } : n
            );
          }
          ...
          if (method === "subscription.import" || method === "subscription.refresh") {
            ...
            if (method === "subscription.refresh") {
              ...
              sourceId = (0, import_node_crypto.randomUUID)();
              text = String(
                await this.extension(
                  "subscription.fetch",
                  { url: url.href },
            ...
            if (sourceId) {
              const identity = (n) => [n.type, n.name, n.server, n.port].join("|");
              const previous2 = new Map(
                next.nodes.filter((n) => n.sourceId === sourceId).map((n) => [identity(n), n.id])
              );
              for (const node of nodes) {
                node.id = previous2.get(identity(node)) ?? node.id;
              }
            }
            next.nodes = next.nodes.filter((n) => !sourceId || n.sourceId !== sourceId).concat(nodes);
            if (sourceId) {
            ...
            if (next.settings.selectedNode !== "direct" && !next.nodes.some((n) => n.id === next.settings.selectedNode) && !(next.externalNetworks ?? []).some(
        ...
        if (extensionsChanged || method === "extensions.setEnabled" || method === "extensions.restart") {
        ...
        Object.assign(operation, {
          state: error.outcome === "unknown" ? "unknown" : "failed",
          completedAt: (/* @__PURE__ */ new Date()).toISOString(),
  ...
  async drain() {
    ...
    await writeHandoff(this.store.directory, {
      ...
      epoch: this.store.epoch,
      revision: this.store.configuration.revision,
      pendingOperationIds: this.store.operations.filter(
...
// packages/service/src/kernel/telemetry.ts
var import_grpc_js2 = __toESM(require_src3(), 1);
var import_proto_loader2 = __toESM(require_src2(), 1);
var import_node_path4 = require("node:path");

// packages/service/src/kernel/traffic-history.ts
var TrafficHistory = class {
  previous;
  points = [];
  sample(at, upload, download) {
    const p = this.previous;
    const elapsed = p ? (at - p.at) / 1e3 : 0;
    const valid = p && elapsed > 0 && elapsed <= 5 && upload >= p.upload && download >= p.download;
    const point = {
      at,
    ...
    while (this.points.length > 600 || this.points[0].at < at - 6e5)
      this.points.shift();
    return point;
  }
  reset() {
...
// packages/service/src/kernel/telemetry.ts
var KernelTelemetry = class {
  constructor(schemaPath = (0, import_node_path4.join)(__dirname, "telemetry.proto")) {
    this.schemaPath = schemaPath;
  }
  schemaPath;
  history = new TrafficHistory();
  client;
  streams = [];
  identity = "";
  generation = 0;
  ...
  connect(control) {
    ...
    const definition = (0, import_grpc_js2.loadPackageDefinition)(
      (0, import_proto_loader2.loadSync)(this.schemaPath, { longs: String, defaults: true })
    );
    const Service = definition.daemon.StartedService;
    const client = new Service(control.endpoint, import_grpc_js2.credentials.createInsecure(), {
      "grpc.max_receive_message_length": 4 * 1024 * 1024
    });
    this.client = client;
    const metadata = new import_grpc_js2.Metadata();
    metadata.set("authorization", "Bearer " + control.secret);
    const status = client.subscribeStatus(
      { interval: "1000000000" },
      metadata
    );
    status.on("data", (data) => {
    ...
    const connections = client.subscribeConnections(
      { interval: "1000000000" },
      metadata
    );
  ...
  snapshot() {
    return {
      ...this.state,
      available: this.state.available && Date.now() - (this.state.sampledAt ?? 0) < 5e3,
...
function capability(method, payload, signal) {
...
var core = new ServiceCore(
  new StateStore(process.env.FLOWGATE_DATA, epoch),
  {
    ...
    apply: (config, revision, operationId, mode) => capability("native.apply", { config, revision, operationId, mode }),
    stop: (operationId) => capability("native.stop", { operationId })
  },
  (method, payload, signal) => capability("extension.call", { method, payload }, signal),
  process.env.FLOWGATE_RELEASE,
  new KernelTelemetry(),
  process.env.FLOWGATE_HOST_VERSION
);
var ready = process.env.FLOWGATE_PREFLIGHT === "1" ? core.store.start(true).then(() => {
  core.lifecycle = "ready";
}) : core.start();
ready.catch(() => {
});
var requests = new RequestScope();
port.on("message", async ({ data }) => {
  ...
  try {
    ...
    if (data.method === "drain") {
      ...
      if (process.env.FLOWGATE_PREFLIGHT === "1" && !["health", "snapshot"].includes(data.method))
        throw new Error("\u53EA\u8BFB\u9884\u68C0\u62D2\u7EDD\u5199\u5165");
      result = await requestTrace.run(
        data.context,
        () => requests.run(
          data.id,
          (signal) => core.request(data.method, data.payload, data.operationId, signal)
        )
      );
    }
...
setInterval(() => {
  ...
  publishing = true;
  void ready.then(() => core.snapshot()).then(
    (snapshot) => port.postMessage({ type: "snapshot", epoch, session, snapshot })
  ).catch(() => {
