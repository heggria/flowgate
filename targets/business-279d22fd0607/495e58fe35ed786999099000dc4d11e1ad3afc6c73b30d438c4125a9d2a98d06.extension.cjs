...
// packages/extensions/src/ruleset.ts
var import_node_crypto = require("node:crypto");
function parseRuleSet(payload) {
  if (typeof payload.text !== "string" || Buffer.byteLength(payload.text) > 4 * 1024 * 1024)
  ...
  const add = (kind, value) => {
    ...
    seen.add(key);
    if (rules.length >= 1e4) throw new Error("\u89C4\u5219\u96C6\u8D85\u8FC7\u6761\u76EE\u9650\u5236");
    rules.push({
      id: (0, import_node_crypto.createHash)("sha256").update(payload.sourceId + ":" + key).digest("hex").slice(0, 32),
      kind,
      value,
  ...
  if (payload.format === "domain-list") {
    for (const line of payload.text.split(/\r?\n/)) {
      ...
      if (!/^(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?\.)+[a-zA-Z0-9-]+$/.test(
        value
      ))
    ...
    for (const rule of parsed.rules) {
      ...
      for (const [kind, values] of Object.entries(rule)) {
        if (!["domain", "domain_suffix", "ip_cidr", "process_name"].includes(kind))
...
function parseDns(text) {
  return text.split(/resolver #\d+/).slice(1).map((block) => ({
    domain: block.match(/^\s*domain\s*:\s*(.+)$/m)?.[1]?.trim() ?? "\u9ED8\u8BA4 / \u4F5C\u7528\u57DF\u89E3\u6790\u5668",
    servers: [...block.matchAll(/nameserver\[\d+\]\s*:\s*(\S+)/g)].map(
      (m) => m[1]
    )
  })).filter((r) => r.servers.length > 0);
}
function parseSystemProxies(text) {
  const result = [];
  for (const [key, kind] of [
    ...
    if (enabled && host && !/[\s/@]/.test(host) && port2 > 0 && port2 <= 65535)
      result.push({ kind, host, port: port2 });
  }
  return result;
}
async function inspectSystem(signal) {
  ...
  return {
    ...
    interfaces: Object.entries((0, import_node_os.networkInterfaces)()).map(([name, items]) => ({
      name,
    ...
    proxyEnabled: results[1].status === "rejected" ? null : /(?:HTTP|HTTPS|SOCKS|ProxyAutoConfig)Enable\s*:\s*1/.test(value(1)),
    systemProxies: parseSystemProxies(value(1)),
    pacEnabled: /ProxyAutoConfigEnable\s*:\s*1/.test(value(1)),
    ipv6Routes: value(4).split("\n").filter(
      (line) => line.trim() && !/^(Routing|Internet6|Destination)/.test(line)
    ).slice(0, 100),
    dns: parseDns(value(2)),
    routes: value(3).split("\n").filter(
      (l) => /^(default|0\/1|128\.0\/1|100\.|10\.|172\.|192\.168)/.test(l.trim())
    ).slice(0, 60),
    warnings
  };
}
...
async function running(name, signal) {
  try {
...
// packages/extensions/src/parser.ts
...
function parseSubscription(text, sourceId) {
  ...
  const make = (o) => ({
    ...
    type: String(o.type) === "ss" ? "shadowsocks" : String(o.type) === "socks5" ? "socks" : String(o.type),
    server: String(o.server ?? ""),
    port: Number(o.server_port ?? o.port),
    options: Object.fromEntries(
      Object.entries(o).filter(
        ([k]) => !["tag", "name", "server", "server_port", "port", "type"].includes(k)
      )
    ),
    sourceId
  });
  if (input.startsWith("{") || input.startsWith("[")) {
    ...
    nodes = entries.filter(
      (o) => !["direct", "block", "selector", "urltest", "dns"].includes(o.type)
    ).map(make);
  } else if (/^proxies\s*:/m.test(input)) {
    const data = (0, import_yaml.parse)(input, { maxAliasCount: 50 });
    if (!Array.isArray(data.proxies)) throw new Error("\u7F3A\u5C11 proxies");
    nodes = data.proxies.map((o) => {
      const n = make(o);
      ...
      if (o["alterId"] !== void 0) n.options.alter_id = o.alterId;
      return n;
    });
  } else {
    let lines = input;
    ...
    nodes = lines.split(/\r?\n/).filter(Boolean).map((line) => {
      if (line.startsWith("vmess://")) {
        ...
        if (!["tcp", "ws", "grpc", void 0, ""].includes(p.net))
        ...
        return {
          ...
          options: {
            ...
            ...p.net === "grpc" ? { transport: { type: "grpc", service_name: p.path || "" } } : {}
          }
        };
      }
      ...
      const n = {
        ...
        type: type === "ss" ? "shadowsocks" : type === "socks5" ? "socks" : type,
        server: u.hostname,
        port: Number(u.port) || (["http", "socks", "socks5"].includes(type) ? 1080 : 443),
        sourceId,
        options: {}
      };
      if (u.searchParams.get("security") && !["tls", "none", "reality"].includes(u.searchParams.get("security")))
        throw new Error("\u4E0D\u652F\u6301\u7684 TLS \u6A21\u5F0F");
      if (type === "ss") {
        const auth = u.password ? decodeURIComponent(u.username) + ":" + decodeURIComponent(u.password) : u.username.includes(":") ? decodeURIComponent(u.username) : Buffer.from(
        ...
        n.options = {
          ...
          ...u.searchParams.get("flow") ? { flow: u.searchParams.get("flow") } : {}
        };
      } else {
        n.options = {
  ...
  if (!nodes.length || nodes.length > 5e3)
...
var TraceBuffer = class {
  ...
  emit(name, status, context = {}, detail) {
...
var ModuleRuntime = class {
  constructor(trace = new TraceBuffer(), context = () => ({})) {
    this.trace = trace;
  ...
  async activate(modules, satisfiedDependencies = /* @__PURE__ */ new Set()) {
    ...
    try {
      for (const module2 of ordered) {
        const entry = {
          module: module2,
          controller: new AbortController(),
          disposers: [],
          identity: {
            pluginInstance: module2.manifest.id + ":" + crypto.randomUUID(),
            moduleVersions: { [module2.manifest.id]: module2.manifest.version }
          }
        };
        this.active.push(entry);
        this.trace.emit(module2.manifest.id, "starting", {
          ...this.context(),
          ...entry.identity
        });
        await module2.activate({
        ...
        this.trace.emit(module2.manifest.id, "ready", {
          ...this.context(),
          ...entry.identity
        });
      }
...
// packages/extensions/src/runtime.ts
var ExtensionRuntime = class {
  modules = new ModuleRuntime();
  active = /* @__PURE__ */ new Set();
  controller = new AbortController();
  draining = false;
  async start(modules, satisfiedDependencies) {
  ...
  async call(method, payload, signal) {
    ...
    const entry = this.modules.entries.find(
      (entry2) => entry2.kind === "command" && entry2.id === method
    );
    ...
    const pending = Promise.resolve().then(
      () => entry.value(
        payload,
        signal ? AbortSignal.any([signal, this.controller.signal]) : this.controller.signal
      )
    );
    this.active.add(pending);
    const context = {
      ...requestTrace.getStore(),
      pluginInstance: (entry.owner?.id ?? "extension") + ":" + (process.env.FLOWGATE_EPOCH ?? "local"),
      ...entry.owner ? { moduleVersions: { [entry.owner.id]: entry.owner.version } } : {}
    };
  ...
  async stop() {
    try {
      ...
      await beforeDeadline(this.modules.stop(), Date.now() + 5e3, "\u6269\u5C55\u91CA\u653E");
    }
  }
};
...
var ExtensionManager = class {
  constructor(descriptors, modules, identity) {
    this.identity = identity;
    const methods = /* @__PURE__ */ new Set();
    for (const descriptor of descriptors) {
      const candidates = modules.filter(
        (module3) => module3.manifest.id === descriptor.id
      );
      if (this.entries.has(descriptor.id) || candidates.length !== 1)
        throw new Error("\u6269\u5C55\u5B9A\u4E49\u91CD\u590D\u6216\u5B9E\u73B0\u7F3A\u5931");
      const module2 = candidates[0];
      if (module2.manifest.version !== descriptor.version || module2.manifest.api !== 1 || JSON.stringify(module2.manifest.dependencies) !== JSON.stringify(descriptor.dependencies) || JSON.stringify(module2.manifest.capabilities) !== JSON.stringify(descriptor.capabilities))
  ...
  snapshot() {
    return this.order.map((id) => {
      ...
      return {
        ...
        instance: entry.generation ? this.instance(entry) : void 0,
        error: entry.error
      };
    });
  }
  ...
  async enable(entry) {
    ...
    entry.runtime = runtime2;
    runtime2.modules.trace.onChange = () => {
      const event = runtime2.modules.trace.snapshot().at(-1);
    ...
    try {
      const dependencies = new Set(
        [...this.entries].filter(([, other]) => other.status === "ready").map(([id]) => id)
      );
      await beforeDeadline(
        runtime2.start([entry.module], dependencies),
        Date.now() + 5e3,
        "\u6269\u5C55\u6FC0\u6D3B"
      );
      if (this.closing) throw new Error("\u6269\u5C55\u5BBF\u4E3B\u6B63\u5728\u9000\u51FA");
      const methods = runtime2.modules.entries.map((contribution) => contribution.id).sort();
      if (runtime2.modules.entries.some(
        (contribution) => !entry.descriptor.contributions.includes(contribution.kind) || contribution.kind === "command" && typeof contribution.value !== "function"
      ) || JSON.stringify(methods) !== JSON.stringify([...entry.descriptor.methods].sort()))
      ...
      if (runtime2.modules.status === "starting") entry.unsafe = true;
      try {
        await runtime2.stop();
      } catch {
        entry.unsafe = true;
  ...
  async call(method, payload, signal) {
    ...
    const entry = [...this.entries.values()].find(
      (entry2) => entry2.descriptor.methods.includes(method)
    );
  ...
  async inspectNetwork(signal) {
    ...
    const candidates = this.snapshot().filter(
      (entry) => entry.status === "ready" && entry.methods.some((method) => method.startsWith("diagnostic."))
    );
    const reports = await Promise.allSettled(
      candidates.map((entry) => this.call(entry.methods[0], void 0, signal))
    );
...
// packages/extensions/src/host.ts
...
if (process.env.FLOWGATE_EXPECTED_EXTENSIONS) {
  const expected = JSON.parse(process.env.FLOWGATE_EXPECTED_EXTENSIONS);
  if (builtinExtensions.some((entry) => {
    const declared = expected.find((item) => item.id === entry.id);
    return declared?.version !== entry.version || ["permissions", "capabilities", "contributions"].some(
      (key) => JSON.stringify(
        [...declared?.[key] ?? []].sort()
      ) !== JSON.stringify([...entry[key]].sort())
    );
  }))
  ...
  },
  {
    ...
    async activate(context) {
      context.contribute({
        id: "network.inspect",
        kind: "command",
        value: (_, signal) => inspectSystem(signal)
      });
    }
  },
  {
    ...
    async activate(context) {
      context.contribute({
        ...
        kind: "command",
        value: (payload) => parseSubscription(payload.text, payload.sourceId)
      });
      context.contribute({
        id: "subscription.fetch",
        kind: "command",
        value: async (payload, signal) => {
  ...
  builtins.push({
    ...
    async activate(context) {
      context.contribute({
        ...
        kind: "command",
        value: (_, signal) => plugin.inspect(signal)
      });
    }
  });
}
var runtime = new ExtensionManager(builtinExtensions, builtins, {
  epoch,
  releaseSet: process.env.FLOWGATE_RELEASE ?? "bundled",
  version: process.env.FLOWGATE_HOST_VERSION ?? "unknown"
});
runtime.trace.onChange = () => port.postMessage({
  type: "trace",
  epoch,
  session,
  event: runtime.trace.snapshot().at(-1)
});
var ready = runtime.configure({}, true);
ready.catch(() => {
});
var requests = new RequestScope();
port.on("message", async ({ data }) => {
  if (data?.type === "power") return;
  if (data?.type === "cancel") {
  ...
  try {
    ...
    if (data.method === "health")
      ...
      await runtime.drain(Date.now() + 5e3);
      result = { drained: true };
      ...
      result = await requestTrace.run(
        ...
        () => requests.run(
          data.id,
          (signal) => data.method === "network.inspect" ? runtime.inspectNetwork(signal) : runtime.call(data.method, data.payload, signal)
        )
      );
