...
// packages/runtime/src/pausable-timers.ts
var PausableTimers = class {
  constructor(now = () => performance.now(), schedule = setTimeout, clear = clearTimeout) {
    this.now = now;
...
var MockGateway = class {
  constructor(token2, egress, configurationRevision = 1, stressChunks = 0, host2, upstream2) {
    this.token = token2;
    this.egress = egress;
  ...
  async start() {
    ...
    this.server = (0, import_node_http.createServer)(async (req, res) => {
      ...
      try {
        ...
        this.lastFailure = error instanceof Error ? error.message.slice(0, 300) : "Gateway request failed";
        if (!res.headersSent)
...
var capabilityTimers = new PausableTimers();
...
var upstream = process.env.FLOWGATE_INTERNAL_UPSTREAM;
var egressId = upstream ? "flowgate.policy" : "test.explicit";
var host = new CapabilityHost(
  manifest,
  /* @__PURE__ */ new Set([egressId]),
  /* @__PURE__ */ new Set(["provider.mock"]),
  (reference) => hostRequest("credential.resolve", { reference }),
  upstream ? (id) => hostRequest("egress.resolve", { id }) : void 0
);
...
port.on("message", async ({ data }) => {
  if (data?.type === "power") {
    if (data.protocol === 1 && data.epoch === epoch && data.session === session) {
