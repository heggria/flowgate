...
export function validateReleaseManifest(manifest: ReleaseSet) {
  if (
    ...
    manifest.shellApi.max < manifest.shellApi.min ||
    manifest.schema.min < 1 ||
    manifest.schema.max < manifest.schema.min ||
    manifest.revoked
  )
    throw new Error("版本清单格式无效或已撤回");
  ...
  if (
    manifest.catalogVersion !== undefined &&
    (!Number.isSafeInteger(manifest.catalogVersion) ||
      manifest.catalogVersion < 1)
  )
    throw new Error("扩展目录版本无效");
  if (
  ...
  if (!Array.isArray(manifest.builtins) || manifest.builtins.length > 2000)
    throw new Error("内置模块清单无效");
  const identities = new Set<string>();
  for (const builtin of manifest.builtins) {
    if (
      !builtin ||
      typeof builtin.id !== "string" ||
      !/^[\w.-]{1,100}$/.test(builtin.id) ||
      identities.has(builtin.id) ||
      typeof builtin.version !== "string" ||
      !/^[\w.+-]{1,80}$/.test(builtin.version)
    )
      throw new Error("内置模块版本无效");
    identities.add(builtin.id);
