...
新增回归用例复现了实际实现缺陷：正在运行系统代理时，用户保存但尚未应用手动模式；第三方随后接管 HTTP 代理，原协调逻辑会先因修订不一致返回，未停止仍在运行的旧系统代理。失败证据为 `work/readiness/network-draft-yield-before.log`。保存后的设置不等于正在生效的连接模式。

Service 现在用精确 operationId/revision 对应的已应用连接判断系统代理所有权丢失；这种情况下只停止原连接，保留未应用设置。普通路径变化遇到未应用配置仍提示而不自动应用。串行执行前继续核对原生操作和修订，断开记录使用已应用模式，避免把真实系统代理写成手动模式。110 项单元测试、类型、边界、构建、恢复界面及草稿验证通过；新增临时 CI 用例将实际修改路由，验证自动重建、未应用修改保留及第三方接管后的停止，真实结果尚待新提交检查。

此前 `e6dd3a9` 的两套完整检查与 15 项真实特权场景已经通过。现有 CI818 包及其正在运行的长测不包含本节修复，不能作为新行为的验收证据。

## 真实路由验收发现的采集缺陷

`8c2d760` 的两套检查在 `service-route-add` 失败：原生路由已经添加，但 Service 没有自动重连。此前 13 个特权场景通过，最后卸载及网络恢复通过；后续路由删除、未应用配置接管和完整 TUN 场景没有完成，不能写成全套通过。分支精确失败记录为 `work/readiness/ci8c-privileged-result.json`，PR 合并提交记录另存。

定位到实际路由采集函数只保留默认路由及部分私网前缀，因而丢弃测试中的 `198.18.2.0/24`，也会遗漏公网分流路由；60 行截断还可能让临时邻居条目挤掉后续静态路由。已改为识别 IPv4 路由行，保留公网、测试网段和静态主机路由，按路由标记排除临时邻居/克隆条目。命令输出仍受原有字节上限保护。修复前两个回归用例失败，修复后 112 项单元测试、类型、边界、构建与功能界面检查通过；新增失败诊断会记录 Service 是否看到测试路由及最近操作，不影响测试断言。
