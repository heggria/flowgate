...
工作流另有显式的隔离特权验收步骤 `tests/privileged-ci.integration.ts`，仅允许临时 GitHub-hosted macOS 机器执行，使用生产安装器和真实系统设置/进程，不在用户 Mac 上自动运行。`74d88ab` 已通过系统代理/定向 TUN 各三项真实转发及退出/崩溃恢复、三个独立双 TUN 场景、失败替换保留及同版本重装，共 11 项；重复卸载后网络设置一致。具体证据见 READINESS_2026-09-15.md；后续代码需各自验证。该范围不代表整机 TUN、第三方 VPN 共存或用户 Mac 的管理员交互已验收。
...
Developer ID 同团队签名、公证及 Squirrel.Mac 完整应用正式安装更新此前按用户要求延期；对外正式发布前仍须补齐适用的交付验证。本机 helper 安装和管理员批准不要求 Apple 证书，隔离 CI 的真实系统代理/定向 TUN 六项特权矩阵已通过，用户机器和完整共存矩阵仍独立待验，不归入“证书阻塞”。现有 VPN 保持不变，涉及管理员批准或真实网络切换的项目需在具体方案准备完毕后执行。完整 v1 尚不能标记全部通过。
