...
最新复核见 [2026-09-15 第二轮完整复核](REVIEW_2026-09-15.md)，持续发布标准见 [RELEASE_READINESS.md](RELEASE_READINESS.md)。下文各日期历史证据不能直接用于证明当前构建。0.3.2 已支持管理员批准的免费本机辅助服务，使用方式见 [LOCAL_DEVELOPMENT.md](LOCAL_DEVELOPMENT.md)。证书延期仅适用于 Apple 签名正式分发；本机特权能力及剩余实测边界以最新复核为准。
...
Developer ID 同团队签名、公证及 Squirrel.Mac 完整应用正式安装更新此前按用户要求延期；对外正式发布前仍须补齐适用的交付验证。本机 helper 安装和管理员批准不要求 Apple 证书，真实系统代理/TUN 与特权崩溃矩阵应作为独立未完成项，不再归入“证书阻塞”。现有 VPN 保持不变，涉及管理员批准或真实网络切换的项目需在具体方案准备完毕后执行。完整 v1 尚不能标记全部通过。
