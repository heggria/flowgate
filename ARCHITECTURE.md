...
## Subscription conversion boundary

[SUBSCRIPTIONS.md](SUBSCRIPTIONS.md) specifies the version 2 fetch/document/preview contracts. The verified Extension Host owns bounded parser workers; adapters emit declarative normalized data. Service alone plans optional profile replacement and commits one-time, expiring previews bound to an exact configuration fingerprint. Metadata-only HTTP 304 commits preserve the network revision without weakening stale-preview rejection. Opt-in automatic refresh belongs to the Service drain/pause lifecycle.

The 0.3 full application establishes shell API 2 and state schema 2. The admitted writer backs up schema 1 before migrating; read-only release preflight does not write. A normalized-node version marker keeps legacy ignored fields from becoming active on upgrade. The fixed release loader rejects incompatible business code before it can mutate schema 2 state.
