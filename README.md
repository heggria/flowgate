...
侧栏 **扩展** 可查看 5 个内置扩展的用途、版本、实际运行状态、访问范围、依赖和诊断记录。订阅解析、规则集解析、系统网络发现是核心能力；Tailscale 与 GUI.for.SingBox 的状态诊断可以分别启用或停用。偏好会保存，应用重启及业务更新后仍保留，停用诊断不影响代理转发或外部应用。

页面中的官方更新使用经过验证的版本组合，不支持任意插件安装。架构、生命周期和开发接入方式见 [EXTENSIONS.md](EXTENSIONS.md)。
